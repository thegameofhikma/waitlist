export type Lang = "en" | "ar";

export const t = {
  en: {
    heroTitle1: "Welcome to",
    heroTitle2: "Hikma",
    heroSubtitle:
      "Journey across the world of the Islamic Golden Age in an epic new strategy board game. Seek knowledge, forge wisdom, and secure your legacy in an empire on the brink.",
    heroCta: "Join the Waitlist",

    statsMinutes: "Minutes",
    statsPlayers: "Players",
    statsYears: "Years",

    pillarsTitle1: "The House of Wisdom",
    pillarsDesc1:
      "Bayt al-Hikma in Baghdad stands as a beacon of knowledge: scholars, translators, and scientists gather to exchange ideas, preserve manuscripts, and push the boundaries of human knowledge.",
    pillarsTitle2: "Journey the World",
    pillarsDesc2:
      "Travel between the great cities of the medieval world, gathering Knowledge from distant lands and returning to Baghdad to transform your discoveries into Wisdom.",
    pillarsTitle3: "Secure Your Legacy",
    pillarsDesc3:
      "Race against time to complete your grand masterwork before the advancing Mongol armies reach the capital gates. Every choice matters—will your life's work survive, or be lost to the flames?",

    quote: "\u201cIt is the pole of the world, the navel of the earth, and the City of Peace. There is no one possessing knowledge, or insight who does not look toward it and desire to dwell within it.\u201d",
    quoteBody:
      " ",

    waitlistTitle: "The House of Wisdom Awaits",
    waitlistBlurb:
      "Join the inner circle of scholars today. Be the first to know when Hikma launches.",
    waitlistCount1: "scholar has already joined.",
    waitlistCountN: "scholars have already joined.",
    placeholderName: "Your Name",
    placeholderEmail: "Your Email",
    placeholderCountry: "Country of Residence",
    submitIdle: "Join the Waitlist",
    submitPending: "Inscribing\u2026",
    successTitle: "Welcome, Scholar",
    successBody: "We will send word when the time is right.",

    toastSuccessTitle: "Welcome to the House of Wisdom",
    toastSuccessDesc: "Your name has been inscribed in the ledger.",
    toastDupeTitle: "Already Enrolled",
    toastDupeDesc: "This scholar is already on our list.",
    toastErrorTitle: "Error",
    toastErrorDesc: "Could not add your name to the list.",
    toastCopiedTitle: "Link copied!",
    toastCopiedDesc: "Share it with fellow scholars.",

    shareTitle: "Hikma \u2014 The Board Game inspired by the Islmaic Golden Age",
    shareText:
      "Journey across the Islamic Golden Age in this beautifully crafted board game. Join the waitlist!",

    footerDesign: "Game Design",
    footerIllustration: "Illustration",
    footerShare: "Share",
    footerCopy: "2026 Hikma. All rights reserved.",

    langToggle: "العربية",
  },
  ar: {
    heroTitle1: "مرحباً بك في",
    heroTitle2: "حكمة",
    heroSubtitle:
      "ارتحل في عالم العصر الذهبي الإسلامي في لعبة لوحية جديدة عربية المنبت. ابحث عن المعرفة، صُغ الحكمة، وحافظ على إرثك من الزوال!",
    heroCta: "انضم إلى قائمة الانتظار",

    statsMinutes: "دقيقة",
    statsPlayers: "لاعبون",
    statsYears: "سنوات",

    pillarsTitle1: "بيت الحكمة",
    pillarsDesc1:
      "يقف بيت الحكمة في بغداد منارةً للعلم، حيث يجتمع طالبو العلم مع المترجمين والعلماء لتبادل الأفكار، دراسة المخطوطات، وتوسعة حدود المعرفة الإنسانية. ",
    pillarsTitle2: "سافر حول العالم",
    pillarsDesc2:
      "تنقّل بين أعظم مدن العصور القديمة، اجمع العلم من أصقاع الأرض وعد إلى بغداد لتحوّل اكتشافاتك الجديدة إلى حكمة!",
    pillarsTitle3: "رسّخ إرثك",
    pillarsDesc3:
      "تسابق مع الزمن لتكمل بحثك العظيم قبل أن تصل جيوش الغزاة المغول إلى العاصمة. كل قرارٍ تتخذه له عواقبه - هل ستتمكن من الحفاظ على عملك العظيم، أم أنه سيضيع بين ألسنة الذهب؟",

    quote: "«هِيَ قُطْبُ الدُّنْيَا، وَسُرَّةُ الْأَرْضِ، وَمَدِينَةُ السَّلَامِ... لَيْسَ لِأَحَدٍ مِنَ الْعِلْمِ وَالْأَدَبِ وَالْفِقْهِ وَالنَّظَرِ إِلَّا وَهُوَ يَتَطَلَّعُ إِلَيْهَا، وَيَرْغَبُ فِي سُكْنَاهَا»",
    quoteBody:
      " ",
    waitlistTitle: "بيت الحكمة ينتظرك",
    waitlistBlurb:
      "انضم اليوم إلى طالبي العلم، وكن أول من يعلم عند انطلاق «حِكمة»",
    waitlistCount1: "عالم واحد انضم بالفعل.",
    waitlistCountN: "عالم انضموا بالفعل.",
    placeholderName: "اسمك",
    placeholderEmail: "بريدك الإلكتروني",
    placeholderCountry: "بلد الإقامة",
    submitIdle: "انضم إلى قائمة الانتظار",
    submitPending: "جارٍ التسجيل…",
    successTitle: "أهلاً أيها العالم",
    successBody: "سنُرسل إليك خبراً حين يحين الوقت.",

    toastSuccessTitle: "مرحباً بك في بيت الحكمة",
    toastSuccessDesc: "لقد سُجّل اسمك في السجل.",
    toastDupeTitle: "مسجّل بالفعل",
    toastDupeDesc: "هذا العالم موجود في قائمتنا.",
    toastErrorTitle: "خطأ",
    toastErrorDesc: "تعذّر إضافة اسمك إلى القائمة.",
    toastCopiedTitle: "تم نسخ الرابط!",
    toastCopiedDesc: "شاركه مع زملائك العلماء.",

    shareTitle: "حكمة — لعبة الطاولة المستوحاة من العصر الذهبي الإسلامي",
    shareText: "سافر عبر العصر الذهبي الإسلامي في لعبة طاولة مصنوعة بعناية. انضم إلى قائمة الانتظار!",

    footerDesign: "تصميم اللعبة",
    footerIllustration: "الرسم والتصوير",
    footerShare: "مشاركة",
    footerCopy: "لعبة حكمة. جميع الحقوق محفوظة.",

    langToggle: "English",
  },
} satisfies Record<Lang, Record<string, string>>;
